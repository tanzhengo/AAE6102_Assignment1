...
for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
        ...
        for loopCnt =  1:codePeriods
            ...
            I_E03 = sum(earlyCode03  .* iBasebandSignal);
            Q_E03 = sum(earlyCode03  .* qBasebandSignal);
            I_E02 = sum(earlyCode02  .* iBasebandSignal);
            Q_E02 = sum(earlyCode02  .* qBasebandSignal);
            I_E01 = sum(earlyCode01  .* iBasebandSignal);
            Q_E01 = sum(earlyCode01  .* qBasebandSignal);
            I_L01 = sum(lateCode01   .* iBasebandSignal);
            Q_L01 = sum(lateCode01   .* qBasebandSignal);
            I_L02 = sum(lateCode02   .* iBasebandSignal);
            Q_L02 = sum(lateCode02   .* qBasebandSignal);
            I_L03 = sum(lateCode03   .* iBasebandSignal);
            Q_L03 = sum(lateCode03   .* qBasebandSignal);
            I_L04 = sum(lateCode04   .* iBasebandSignal);
            Q_L04 = sum(lateCode04   .* qBasebandSignal);
            end
